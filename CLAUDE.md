...
### Record-type history (sidebar nav)

If your skillbox ships an approval-gated tool that writes through `services.records`, you can expose a browsable history of past records by declaring `recordTypes` under `branding`:
...
The panel paints one sidebar entry per record type. Clicking through fetches `GET /api/records/:userId?type=<type>` and renders each past row using the renderer registered under `toolName` (defaults to `type`). The records-history view re-uses your existing renderer — it synthesises a `{ ...record.data, recorded: true, recordId, recordedAt }` envelope, so if your renderer already handles the live "Recorded · id …" footer it works on history for free.

Aggregation across skillboxes: last-wins per `type`, declaration order preserved (so an upstream skillbox's label can be overridden without reshuffling the sidebar).
...
- **Read tool over the public web, auto-execute.** `web_research` searches the web (Tavily / Brave / DuckDuckGo, provider auto-selected from env at call time) when a question is current-events / vendor-specific / not in the KB. No approval — searching is a read. Returns the same `sources[]` shape as `lookup_regulations` so `renderSourceCards` paints citation cards for free. The renderer (`web_research.js`) wraps the cards with a query-echo header and a `N results · source: <provider>` footer.
